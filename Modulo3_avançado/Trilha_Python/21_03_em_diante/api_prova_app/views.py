...
from api_prova_app.serializers import *
...
def produtos(request):
  if request.method == "POST" and request.user.is_authenticated and "site_app" in request.user.get_all_permition:
    ...
    produtoData = Produto2.objects.order_by('id') 
...
def produto_id(request, produto_id):
  try:
    produtoData = Produto2.objects.get(id=produto_id) 
  except Produto2.DoesNotExist:
